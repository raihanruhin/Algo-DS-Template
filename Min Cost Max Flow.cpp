Min Cost Max Flow, O(n*m*log(n)) [not confirmed]
For each undirected graph, 4 edge needs to be added. For directed, 2 needed.
Tested on: Uva 10594
...
{
    e[edgenum].u = u, e[edgenum].v = v, e[edgenum].cost = c;
    e[edgenum].cap = f,  e[edgenum].next = first [u], first[u] = edgenum;
    edgenum++;
}
...
void SPFA(int node)
{
    ...
    while (!q.empty ())
    {
        ...
        for ( int k = first[u]; k != - 1 ; k = e[k].next)
        {
            ...
            {
                par[v] = k,  dist[v] = dist[u] + e[k].cost;
                if (!vis[v])
                {
                    vis[v] = 1 ;
                    q.push (v);
...
int main ()
{
    ...
    while(scanf("%d%d", &node, &edge)==2)
    {
        ...
        for ( int i = 0 ; i < edge; i ++)         scanf ( "%d%d%lld" , &u[i], &v[i], &cost[i]);
